...
ORGANIZATION_PROTECTED = 'org_protected'
ORGANIZATION_UNPROTECTED = 'org_unprotected'
ORGANIZATION_PROTECTION_ADMIN = 'org_protection_admin'
...
def _is_course_admin(user, course_key):
    """
...
def has_team_api_access(user, course_key, access_username=None):
    ...
    """
    if _is_course_admin(user, course_key):
        return True
...
def user_organization_proctection_status(user, course_key):
    """
    Returns the organization protection status of the user related to this course
    If the user is in the Masters track of the course, we return the proected status.
    If the user is a staff of the course, we return the protection_admin status
    else, we return the unprotected status
    """
    if _is_course_admin(user, course_key):
        return ORGANIZATION_PROTECTION_ADMIN
    enrollment = CourseEnrollment.get_enrollment(user, course_key)
    if enrollment:
        if enrollment.mode == CourseMode.MASTERS:
            return ORGANIZATION_PROTECTED
        else:
            return ORGANIZATION_UNPROTECTED
    else:
        logger.warning("Student [%s] is not enrolled with course [%s] enrollment", user.id, course_key)
        return None
...
def has_specific_team_access(user, team):
    ...
    """
    if _is_course_admin(user, team.course_id):
        return True
    if team.organization_protected:
        return ORGANIZATION_PROTECTED == user_organization_proctection_status(user, team.course_id)
    else:
        return ORGANIZATION_UNPROTECTED == user_organization_proctection_status(user, team.course_id)


class TeamCounter(object):
    """The helper class to provide functions for calculating team count value"""

    @classmethod
    def get_team_count_query_set(cls, topic_id_set, course_id, organization_protection_status):
        ...
        if organization_protection_status != ORGANIZATION_PROTECTION_ADMIN:
            filter_query.update(
                {'organization_protected': organization_protection_status == ORGANIZATION_PROTECTED}
            )
        return CourseTeam.objects.filter(**filter_query)

    @classmethod
    def add_team_count(cls, topics, course_id, organization_protection_status):
        """
        Helper method to add team_count for a list of topics.
        This allows for a more efficient single query.
        """
        topic_ids = [topic['id'] for topic in topics]
        teams_query_set = cls.get_team_count_query_set(
            topic_ids,
            course_id,
