...
class TeamAccessTests(SharedModuleStoreTestCase):
    ...
    @ddt.data(
        ('user_audit', teams_api.ORGANIZATION_UNPROTECTED),
        ('user_masters', teams_api.ORGANIZATION_PROTECTED),
        ('user_staff', teams_api.ORGANIZATION_PROTECTION_ADMIN),
        ('user_unenrolled', None),
    )
    @ddt.unpack
    def user_organization_proctection_status(self, username, expected_protection_status):
        user = self.users[username]
        self.assertEqual(
            expected_protection_status,
            teams_api.user_organization_proctection_status(user, COURSE_KEY1)
        )
    ...
    def test_team_counter_get_team_count_query_set(self, username, expected_count):
        user = self.users[username]
        organization_protection_status = teams_api.user_organization_proctection_status(
            user,
            COURSE_KEY1
        )
        teams_query_set = teams_api.TeamCounter.get_team_count_query_set(
            [self.topic_id],
            COURSE_KEY1,
    ...
    def test_team_counter_add_team_count(self, username, expected_team_count):
        ...
        teams_api.TeamCounter.add_team_count(
            [topic],
            COURSE_KEY1,
            organization_protection_status
        )
