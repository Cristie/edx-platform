...
from lms.djangoapps.teams.api import (
    ORGANIZATION_PROTECTION_ADMIN,
    ORGANIZATION_PROTECTED,
    has_team_api_access,
    user_organization_proctection_status,
...
class TeamsDashboardView(GenericAPIView):
    ...
    def get(self, request, course_id):
        ...
        organization_protection_status = user_organization_proctection_status(request.user, course_key)
        ...
        if organization_protection_status != ORGANIZATION_PROTECTION_ADMIN:
            filter_query['organization_protected'] = organization_protection_status == ORGANIZATION_PROTECTED
...
class TeamsListView(ExpandableFieldViewMixin, GenericAPIView):
    ...
    def post(self, request):
        ...
        organization_protection_status = user_organization_proctection_status(request.user, course_key)
        if organization_protection_status != ORGANIZATION_PROTECTION_ADMIN:
            data['organization_protected'] = organization_protection_status == ORGANIZATION_PROTECTED
    ...
    """
        ...
        **Response Values for GET**
            ...
            If the user is logged in, the response contains the following fields:
                ...
                * organization_protected: Whether the team consist of team members in
                  organization protected groups
...
class TopicListView(GenericAPIView):
    ...
    def get(self, request):
        ...
        organization_protection_status = user_organization_proctection_status(request.user, course_id)
        topics = get_alphabetical_topics(course_module)
        if ordering == 'team_count':
            TeamCounter.add_team_count(topics, course_id, organization_protection_status)
